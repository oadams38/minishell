...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line_signals.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: oladams <oladams@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/26 16:24:39 by oladams           #+#    #+#             */
/*   Updated: 2024/02/14 20:33:42 by oladams          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line_signals(int fd)
{
	...
	while (ret == BUFFER_SIZE && !line_is_complete(line))
	{
		...
		if ((ret == -1 || ret == 0 || g_status.hd_fd == 0) && line)
			free(line);
		if (ret == 0 && g_status.hd_fd != 0)
			g_status.exit = 164;
		if (ret == 0 || ret == -1 || g_status.hd_fd == 0)
			return (NULL);
